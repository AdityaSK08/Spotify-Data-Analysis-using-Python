...
sp_tracks = pd.read_csv('C:/spotify_data_analysis_python_project-main/tracks.csv')
sp_feature = pd.read_csv('C:/spotify_data_analysis_python_project-main/features.csv')
...
sp_tracks_cleaned = sp_tracks.drop(['key', 'mode', 'explicit'], axis=1)

# Keep only numeric columns
sp_tracks_numeric = sp_tracks_cleaned.select_dtypes(include=['number'])

# Compute correlation
td = sp_tracks_numeric.corr(method='pearson')
